...
read -p "do you want to interpolate the files to 6hr (skip if already done) press yes: "  answer

if [[ "$answer" != "yes" ]]; then
	echo "skipping to next stage"
else
	# interpolate to 6-hourly files
	for x in tos_Oday*.nc
	do
	outprefix=`echo $x | cut -d_ -f1`'_6hr_'`echo $x | cut -d_ -f3-6`
...
read -p "files are interpolated to the folder ${outfolder} press yes to continue split/shift time: "  answer
